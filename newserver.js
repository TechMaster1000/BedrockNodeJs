...
const validateChatRequest = (req, res, next) => {
  try {
    const { message, conversation_history = [], max_tokens, temperature } = req.body;
    ...
    for (const msg of conversation_history) {
      if (!msg.role || !['user', 'assistant'].includes(msg.role)) {
        return res.status(400).json({ error: 'Invalid conversation history: each message must have a valid role (user or assistant)' });
      }
      if (!msg.content || typeof msg.content !== 'string') {
        return res.status(400).json({ error: 'Invalid conversation history: each message must have content as a string' });
      }
    }
